...
The Visit screen displays detailed information about a specific care visit and allows the care worker to update visit status and record visit details. It supports offline access with read-only enforcement during the offline grace period.

## Overview

- **Component**: [screen.tsx](./screen.tsx)
- **Purpose**: Display visit details, allow status updates, and record visit outcomes
- **Offline Behavior**: Displays cached visit details in read-only mode if offline and within the 7-day grace period

## Architecture & Integration

### Offline Grace Period (P1-AUTH-004)

The Visit screen respects the offline grace period defined in [ARCHITECTURE.md](../../ARCHITECTURE.md#offline-grace-period-p1-auth-004):
...
2. **UI Indicators**: 
   ...
   - All mutation buttons (complete visit, update status, record notes, etc.) are disabled
   - User can still view cached visit details in full detail
...
5. **Visit data is cached and readable offline** – Apollo cache or local storage provides visit details even if no network

See [src/__tests__/screens/visit.test.tsx](../../__tests__/screens/visit.test.tsx) for test patterns.
...
- **Blueprint Section 5 (Flow 1)**: Visit screen is navigable from Today screen; displays cached visit details.
- **Blueprint Section 2 (Offline Capability)**: Read-only visit details on cached data; write mutations blocked until reconnect.
...
- [Today Screen README](../today/README.md) – Related screen documentation for schedule context
