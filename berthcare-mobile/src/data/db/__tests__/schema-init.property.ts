  ...
  it('runs initialization only once regardless of repeated calls', async () => {
    await fc.assert(
      fc.asyncProperty(fc.integer({ min: 2, max: 5 }), async (runs) => {
        const { getOrCreateEncryptionKey, initialize, CREATE_INDEX_STATEMENTS, CREATE_TABLE_STATEMENTS } = setup();
        ...
        const executedStatements = mockExecuteAsync.mock.calls.map(([, statement]) => statement);

        expect(executedStatements[0]).toBe('PRAGMA key = ?;');
        CREATE_TABLE_STATEMENTS.forEach((statement) => {
          expect(executedStatements.filter((s) => s === statement)).toHaveLength(1);
        });
        CREATE_INDEX_STATEMENTS.forEach((statement) => {
          expect(executedStatements.filter((s) => s === statement)).toHaveLength(1);
        });
        expect(executedStatements.filter((s) => s.includes('schema_version')).length).toBeGreaterThan(0);
        expect(executedStatements.filter((s) => s.startsWith('INSERT INTO schema_version')).length).toBe(1);
      }),
      { numRuns: 50 }
    );
  });
