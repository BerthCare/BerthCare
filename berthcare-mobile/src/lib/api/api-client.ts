...
import type { ApiClientConfig, ApiRequestHandle, ApiResponse, RequestOptions, TokenProvider, HttpMethod } from './types';
...
export class ApiClient {
  ...
  post<T>(path: string, data?: unknown, options?: RequestOptions): ApiRequestHandle<ApiResponse<T>> {
  ...
  patch<T>(path: string, data?: unknown, options?: RequestOptions): ApiRequestHandle<ApiResponse<T>> {
  ...
  private request<T>(method: HttpMethod, path: string, data?: unknown, options?: RequestOptions): ApiRequestHandle<ApiResponse<T>> {
    const controller = new AbortController();
    const promise = options?.skipRetry
      ? this.performRequest<T>(method, path, data, options, controller, true)
      : executeWithRetry(
          (attempt) => this.performRequest<T>(method, path, data, { ...options, attempt }, controller, true),
    ...
    try {
      ...
      if (response.status === 401 && allow401Refresh) {
        return handle401Response(this.tokenProvider, () =>
          this.performRequest(method, path, data, { ...options, attempt: (options?.attempt ?? 0) + 1 }, abortController, false),
        );
      }
